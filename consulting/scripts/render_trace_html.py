#!/usr/bin/env python3
"""可信核验报告渲染器：把统一问答答案/材料 JSON 渲染为单文件可核验 HTML。

报告定位：不止展示"材料从哪来"（溯源），更要传达"我们查过了，可以交付"（核验）。
v1.3.0 架构（对齐公文写作 3.7.0 交互重构，数据口径保持咨询接口自有与真实）：
...
tabs；另保留咨询自有的云端溯源存档（traceUrl）区与生成前硬校验。
"""
...
# ============================================================
# 工作区路径（official-docs 约定）与咨询自有数据处理
# ============================================================
...
def strip_trailing_source_list(text: str) -> str:
    ...
    来源清单只应在对话输出中展示：核验报告由正文溯源卡与材料专库承载来源，
...
def normalize_citation_punctuation(text: str) -> str:
    """引文胶囊位置规范：角标与后随标点整体换位——标点紧跟文字，胶囊放标点之后。

    对齐深知晓原型"…内容。～出处"形态，避免胶囊把句号挤到下一行孤悬。
    与公文写作 3.7.0 的差异：公文只换单个角标（`[1]。`→`。[1]`），连续角标连排
    （`[1][2]。`，咨询接口答案常见）时标点会悬在胶囊之间；这里整段角标连排
    整体换位（`[1][2]。`→`。[1][2]`），保证标点始终紧贴文字。
    """
    return re.sub(
        r"(?:\[\d+\][ \t]*)+([。；，、！？：；,.])",
...
def resolve_citation_targets(answer: str, sources: List[Dict[str, str]]) -> Dict[str, str]:
    """把答案每个角标解析到材料旧 id：{角标: 材料id}。

    现统一问答接口的真实形态：材料不返回材料级编号，但 content 每段携带
    全局唯一引文编号（id，如 101/135/201…），答案角标即段落编号——因此
    优先按"段落id→材料"解析；同一材料被多段引用时塌缩为该材料一张卡。
    旧接口形态：材料自带 explicit_id（角标即材料编号）按材料级 id 匹配。
    两种形态的未命中角标不入映射，由生成前硬校验拦截。
    """
    para_owner: Dict[str, str] = {}
    for source in sources:
        for pid in (source.get("para_ids") or []):
            para_owner.setdefault(str(pid), source["id"])
    source_ids = {s["id"] for s in sources}
    targets: Dict[str, str] = {}
    for cid in citation_ids(answer):
        if cid in para_owner:
            targets[cid] = para_owner[cid]
        elif cid in source_ids:
            targets[cid] = cid
    return targets


def renumber_citations(answer: str, sources: List[Dict[str, str]]) -> tuple:
    """把答案角标绑定材料并按首次出现顺序重编号为 1..n，未引用材料顺延编号。

    三种数据形态（resolve_citation_targets 统一解析）：
    ① 段落全局编号（现接口）——段落 id 即角标，绑定到所属材料卡；
    ② 材料级编号（旧接口 explicit_id）——角标即材料编号，一一对应；
    ③ 纯顺序兜底（如手工样例：材料 id 为 1..n、角标同为 1..n）。
    交付视角统一使用连续编号 1..n；未绑定角标保持原样，由生成前硬校验拦截。
    返回（重编号后答案, 重编号后来源列表, 旧编号→新编号映射）。
    """
    targets = resolve_citation_targets(answer, sources)
    # 按答案角标首次出现顺序，为每个被引用材料分配连续新编号
    mat_to_new: Dict[str, str] = {}
    para_to_new: Dict[str, str] = {}
    for cid in citation_ids(answer):
        mat_id = targets.get(cid)
        if mat_id is None:
            continue
        if mat_id not in mat_to_new:
            mat_to_new[mat_id] = str(len(mat_to_new) + 1)
        para_to_new[cid] = mat_to_new[mat_id]

    # 未引用材料顺延编号（保持材料卡列表完整，编号不与正文冲突）
    for source in sources:
        old = source["id"]
        if old not in mat_to_new:
            mat_to_new[old] = str(len(mat_to_new) + 1)

    for source in sources:
        if source["id"] in mat_to_new:
            source["id"] = mat_to_new[source["id"]]

    def repl(match: "re.Match[str]") -> str:
        cid = match.group(1)
        if cid in para_to_new:
            return f"[{para_to_new[cid]}]"
        return f"[{mat_to_new.get(cid, cid)}]"

    answer = re.sub(r"\[(\d+)\]", repl, answer)
    # 标点紧跟文字、胶囊放标点之后（含连续角标连排整体换位），
    # 使对话答案、核验报告、干净 Markdown 三处标点位置一致
    answer = normalize_citation_punctuation(answer)
    # 合并映射：段落编号（现接口）与材料编号（旧接口）分别列出，供 Agent 输出映射表
    mapping = {**mat_to_new, **para_to_new}
    return answer, sources, mapping
...
# 注：无素材类型体系——统一问答接口不返回类型字段（类型是公文写作侧的自建概念，
# 咨询版已定稿移除），核验单为四项，来源卡不带类型徽章。
...
def source_from_article(item: Dict[str, Any], index: int, segment: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    for seg in content_segments(item):
        ...
        segments.append({"id": seg.get("id"), "chain": chain_lvls, "text": strip_leading_chain(seg_text, chain_lvls)})
    source["segments"] = segments
    # 段落全局引文编号（现统一问答接口形态：材料不返回材料级编号，content 每段
    # 携带全局唯一 id，答案角标即段落编号）。保留给绑定层做"段落id→材料"解析；
    # 旧接口形态段落 id 每篇从 1 重排，绑定层优先材料级编号，不受此影响。
    source["para_ids"] = [str(seg["id"]) for seg in segments if str(seg.get("id") or "").strip()]
    # 首段章节位置（兼容旧单行引用处）
    ...
    source["conf"] = first_str(item.get("createDateReliability"), item.get("发布日期可信度"), item.get("时间可靠性"), item.get("可信度"), item.get("confidence"), item.get("reliability"))
...
SELF_CHECK_ITEMS = [
    ("citation_present", "角标存在"),
    ("citation_bound", "角标对应来源"),
    ("statement_verifiable", "结论可核验"),
    ("answer_consistent", "答案一致"),
    ("source_list_covered", "来源清单覆盖"),
]

SELF_CHECK_LABELS = dict(SELF_CHECK_ITEMS)
SELF_CHECK_LABELS.update({
    "角标存在": "角标存在", "角标对应来源": "角标对应来源", "结论可核验": "结论可核验",
    "答案一致": "答案一致", "来源清单覆盖": "来源清单覆盖",
})
...
def compute_verification(answer: str, sources: List[Dict[str, str]], payload: Dict[str, Any],
    ...
    # 政策效力：无法自动判定现行效力，列出建议人工复核（咨询无类型体系，按被引用材料数计）
    policy_count = len(cited_sources)
    ...
    if no_citation:
        reasons.append("答案未包含来源角标，无法建立引用-材料对应")
    if unbound:
        reasons.append(f"{len(unbound)} 处答案角标未绑定材料")
    if self_check["status"] == "fail":
        reasons.append("交付前检查存在未通过项")

    return {
        "overall": {
            "passed": overall_passed,
            "label": "核验完成，答案依据逐条对过原文" if overall_passed else "核验未完全通过",
            "reasons": reasons,
        },
...
def compute_process_stats(sources: List[Dict[str, str]], answer: str, question: str = "") -> Dict[str, Any]:
    """过程回顾条数据：问答次数、召回/采用数、入库与引用计数。

    统一问答为单次接口召回，无检索条件分组字段——把用户问题作为唯一一路
    "检索条件"如实展示（召回/采用数真实统计），不虚构分次检索流程。
    """
    cond_rows = []
    if question.strip():
        cond_rows.append({
            "key": question.strip(),
            "total": len(sources),
            "used": sum(1 for s in sources if s.get("used", True)),
        })
...
def parse_answer_blocks(answer: str, valid_ids: set, chip_map: Optional[Dict[str, str]] = None) -> Tuple[List[Dict[str, Any]], set]:
    ...
    def jb_html(cid: str) -> str:
        ...
        if not jb:
            return ""
        if jb.get("url"):
            # 咨询接口无存档快照：原文失效时回退链接是知识专库，文案如实区分
            if jb.get("dead") and jb.get("snapshot"):
                btn_txt = "查看存档全文"
            elif jb.get("dead"):
                btn_txt = "知识专库原文"
            else:
    ...
    while idx < len(lines):
        ...
        if not block:
            idx += 1
            continue
        # 引文胶囊位置规范：标点紧跟文字、胶囊放标点之后（含连续角标连排整体换位）
        block = normalize_citation_punctuation(block)
...
def build_chip_data(sources: List[Dict[str, str]]) -> Dict[str, Dict[str, str]]:
    ...
    for source in sources:
        ...
        if source.get("link_dead"):
            # 咨询接口无存档快照：原文失效时改用知识专库链接回看，两者都缺则不出链接
            url = source.get("snapshot") or source.get("policy_url") or ""
...
def render_verify_panel(v: Dict[str, Any]) -> str:
    ...
    if not tr["total"]:
        ...
        tr_html = (f'<div class="vi"><span class="s ok">✓ 依据溯源 {tr["passed"]}/{tr["total"]}</span>'
                   f'<span class="d">每条材料摘录可比对，原文链接可回看</span></div>')
    else:
    ...
    if bd.get("no_citation"):
        bd_html = (f'<div class="vi"><span class="s fail">✗ 引用对应 0/0</span>'
                   f'<span class="d">答案里没有来源角标，{v["traceability"]["total"]} 条材料对不上答案</span></div>')
    elif not bd["total"]:
        bd_html = '<div class="vi"><span class="s none">— 引用对应 0/0</span><span class="d">答案里没有来源角标</span></div>'
    elif not bd["unbound"]:
        bd_html = (f'<div class="vi"><span class="s ok">✓ 引用对应 {bd["bound"]}/{bd["total"]}</span>'
                   f'<span class="d">答案每处 [n] 都能找到对应材料</span></div>')
    else:
...
def render_source_card(source: Dict[str, str], for_print: bool = False) -> str:
    ...
    if for_print:
        ...
        return f'<article class="scard print-only">{body_html}</article>'
    unused_attr = "" if source.get("used", True) else ' data-unused="1"'
    return (
        f'<article class="scard" id="src-{esc(source["id"])}" '
        f'data-cite-id="{esc(source["id"])}"{unused_attr}>{body_html}</article>'
    )
...
def render_process_bar(stats: Dict[str, Any], verification: Dict[str, Any]) -> str:
    ...
    detail_rows = "".join(
        ...
        f'<span>召回 {row["total"]} 条 · 答案采用 {row["used"]} 条</span></div>'
        for row in stats["conds"]
    ) or '<div class="pd-row"><span>本次问答未记录用户问题</span></div>'
    detail_hint = f'{esc(short(first_query, 26))}…' if (stats["n_searches"] > 1 and first_query) else "查看召回明细"
    ...
    step4_sub = "结论：答案依据逐条对过原文" if ov["passed"] else esc(short("；".join(ov["reasons"]) or "见核验报告单", 30))
    return f"""
    <section class="process" aria-label="生成过程回顾">
      ...
      <ol class="p-steps">
        <li class="p-step expandable" id="p-step-search" role="button" tabindex="0" title="展开各路检索明细">
          ...
          <div class="p-txt"><b>可信问答 {stats['n_searches']} 次</b><i>{step1_sub}</i></div>
        </li>
        <li class="p-arrow" aria-hidden="true">›</li>
        <li class="p-step">
          <span class="p-no">2</span>
          <div class="p-txt"><b>材料入库 {stats['n_materials']} 条</b><i>答案采用 {stats['used_count']} · 召回未采用 {stats['unused_count']}</i></div>
        </li>
        <li class="p-arrow" aria-hidden="true">›</li>
        <li class="p-step">
...
def render_library_view(sources: List[Dict[str, str]], hot_terms: List[str]) -> str:
    """材料专库全屏视图：大搜索 + 热词 + 引用状态筛选 tabs + 材料卡列表。

    统一问答接口无检索条件分组字段（search_key），tabs 只有 全部/未引用 两态——
    这是接口能力差异的如实退化，不虚构检索分组。
    """
    if not sources:
    ...
    # tabs = 引用状态筛选（全部 / 未引用），数字真实统计；统一问答无检索分组字段，不虚构分组
    ...
    <section class="view" id="view-library" aria-label="材料专库">
      ...
      <div class="lib-foot">材料来源：深知可信咨询 · 每条摘录均取自原文原段</div>
    </section>"""
...
def extract_trace_archives(payload: Dict[str, Any]) -> List[str]:
    """收集"云端溯源存档"链接。

    统一问答接口不返回可再浏览的知识专库 knowledgeBase 链接，实际提供
    traceUrl——云平台为本次问答生成的接口侧可信溯源报告。两种字段都兼容，
    避免接口字段调整时丢链接。
    """
    content = payload.get("content") if isinstance(payload.get("content"), dict) else payload
    values: List[str] = []
    ...
    trace = first_str(payload.get("traceUrl"), payload.get("trace_url"), content.get("traceUrl"), content.get("trace_url"))
    if trace:
        values.append(trace)
    return list(dict.fromkeys(values))


def render_archive_zone(archive_urls: List[str], source_count: int) -> str:
    """云端溯源存档区：统一问答场景自行设计，不复用搜索版"原始召回存档"文案。

    统一问答接口返回的 traceUrl 是云平台为本次问答生成的接口侧可信溯源
    报告，定位为云端存档：本地报告便于离线查阅与打印归档，云端存档适合
    向他人出示查验，也可在本地文件遗失时兜底；逐条核验以来源卡原文链接为准。
    """
    if not archive_urls:
        return ""
    chips = []
    for index, url in enumerate(archive_urls):
        count_text = f"对应 {source_count} 条来源" if not index else "云端留存"
        chips.append(
            f'<a class="kb-chip" href="{esc(url)}" target="_blank" rel="noopener">'
            f'<span class="kb-label">本次问答 · 接口侧可信溯源报告</span>'
            f'<span class="kb-count">{esc(count_text)} · 云端存档</span>'
            '<span class="kb-arrow">打开 ↗</span></a>'
        )
    return (
        '<div class="kb-zone"><div class="kb-title">云端溯源存档（接口可信溯源报告）</div>'
        '<div class="kb-desc">以下链接为深知云平台对本次问答留存的接口侧可信溯源报告，与本核验报告内容对应：'
        '本地报告便于离线查阅与打印归档；云端存档适合向他人出示查验，也可在本地文件遗失时兜底。'
        '逐条材料的核验仍以来源卡上的"查看原文"直达官方页面为准。</div>'
        f'<div class="kb-list">{"".join(chips)}</div></div>'
    )


def safe_output_filename(question: str, timestamp: datetime, fallback: str = "dknowc_consulting", suffix_ext: str = ".html") -> str:
    ...
    return f"{stem}_可信核验报告_{suffix}{suffix_ext}"
...
PAGE_CSS += """
/* ===== 咨询版新增：咨询问题行 + 云端溯源存档 ===== */
.q-line{margin:0 auto 10px;max-width:860px;font-size:13px;color:#4b4257;background:rgba(255,255,255,.72);
  border:1px solid var(--tint-bd);border-radius:8px;padding:7px 12px;text-align:center;line-height:1.7}
.q-line b{color:var(--brand)}
.kb-zone{margin:16px 0 4px;border-top:1px dashed var(--line-strong);padding-top:12px}
.kb-title{font-size:12.5px;font-weight:800;color:var(--ink)}
...
def render_html(payload: Dict[str, Any], title: str, answer_override: str = "", question_override: str = "",
                ...
                sources_override: Optional[List[Dict[str, str]]] = None) -> str:
    payload = unwrap(payload)
    answer = normalize_citations(answer_override) if answer_override.strip() else extract_answer(payload)
    if sources_override is not None:
        # main 已完成重编号与硬校验，直接使用同一样本源，保证三处编号一致
        sources = sources_override
    else:
        # 直接调用渲染器（未走 main）：先绑定段落/材料级编号并重编号，保证
        # 角标与来源卡一致（与 main 的 sources_override 使用同一绑定逻辑）
        sources = extract_sources(payload)
        answer, sources, _ = renumber_citations(answer, sources)
    used = citation_ids(answer)
    # 咨询溯源 JSON 无"已引用"标记：引用状态以答案角标为准（未引用不计入核验，专库单独分组）
    for s in sources:
        s["used"] = s["id"] in used
    generated_at = generated_at or datetime.now()
    ...
    process_stats = compute_process_stats(sources, answer, question)
    ...
    if sources and not used:
        citation_warning = (
            ...
            '请修正最终答案文件：在关键结论后标注 [1]、[2] 等角标并逐条对应来源材料，'
            '然后重新运行 render_trace_html.py 生成核验报告。</div>'
        )
    ...
    if process_stats["n_searches"]:
        meta_bits.append(f"可信问答 {process_stats['n_searches']} 次")
    if sources:
        meta_bits.append(f"召回材料 {len(sources)} 条")
    if process_stats["n_cite_marks"]:
        meta_bits.append(f"答案引用 {process_stats['n_cite_marks']} 处")
    meta_bits.append(f"生成于 {generated}")
    meta_line = " · ".join(meta_bits)
    question_html = (f'<div class="q-line"><b>咨询问题：</b>{esc(question)}</div>' if question else "")
...
<title>{esc(display_title)} · 可信核验报告</title>
...
<header class="topbar">
  <div class="tb-left">
    <span class="tb-stamp">可信核验报告</span>
...
<div class="hero">
  <div class="r-badge">可信核验报告</div>
  <h1>{esc(display_title)}</h1>
  {question_html}
  <div class="meta">{esc(meta_line)} ｜ 材料来源：深知可信咨询</div>
...
# ============================================================
# 链接活性检测（对齐公文写作 3.7.0：生成时真实检测，客观信号判死）
# ============================================================

# 软 404 关键词：政府站常以 HTTP 200 返回"页面不存在"错误页，状态码识别不了，
# 需读页面标题嗅探。关键词取自公文写作侧实测样例，且仅匹配 <title>
# （政策正文不会出现在标题里），误杀风险极低。
...
def check_link_alive(url: str, timeout: int = 8) -> bool:
    ...
    ① HTTP 404/410；② 软件 404——HTTP 200 但页面标题为典型失效页。
...
def mark_dead_links(sources: List[Dict[str, str]]) -> int:
    """并发检测全部材料的原文链接，404/410 的标记 link_dead=True。返回失效数。

    咨询接口无存档快照（screenShotPath）通道：失效链接如实不出"查看全文"，
    改用知识专库链接（policyUrl）回看，两者都缺则在卡片标注"待补链接"。
    """
    targets = [(i, s["url"]) for i, s in enumerate(sources)
               if (s.get("url") or "").strip() and s["url"] != "接口未返回"]
    if not targets:
        return 0
    ...
    for i, _ in targets:
        if not alive.get(i, True):
            sources[i]["link_dead"] = True
            dead += 1
    return dead


def main() -> None:
    parser = argparse.ArgumentParser(description="生成深知可信咨询可信核验报告 HTML")
    ...
    parser.add_argument("--title", default="可信核验报告", help="页面标题")
    parser.add_argument("--answer-file", help="最终回答正文文件。整理综合后必须传入，确保 HTML 展示的答案与聊天答案一致。")
    parser.add_argument("--self-check-file", help="交付前检查结果 JSON（五项：角标存在/角标对应来源/结论可核验/答案一致/来源清单覆盖），值支持 通过/pass/✓ 等写法。")
    parser.add_argument("--clean-md-output", help="输出干净 Markdown 路径；内容来自同一份最终答案，并移除 [1]、【1】等溯源角标。")
    parser.add_argument("--question", default="", help="用户问题，用于生成咨询问题行与文件名。")
    parser.add_argument("--skip-link-check", action="store_true", help="跳过原文链接活性检测（默认检测：404/410 标记失效，连接失败/403 放行不误杀）")
    args = parser.parse_args()

    payload = load_json(_resolve_input(args.input_json))
    if args.self_check_file:
        self_check_data = load_json(_resolve_input(args.self_check_file))
        holder = payload.get("data") if isinstance(payload.get("data"), dict) else payload
        holder.setdefault("selfCheck", self_check_data)
    answer_override = ""
    if args.answer_file:
        answer_override = _resolve_input(args.answer_file).read_text(encoding="utf-8")
    base_answer = strip_trailing_source_list(answer_override or extract_answer(unwrap(payload)))
    base_answer = normalize_citations(base_answer)

    # 生成前硬校验：核验报告是交付物，交付时必须为核验通过状态。
    # Agent 可修正的问题在这里一律拒绝生成，倒逼先修正再交付；
    # 仅接口未返回原文链接、政策效力无法自动判定等不可抗因素由报告温和提示。
    pre_sources = extract_sources(unwrap(payload))
    pre_cited = citation_ids(base_answer)
    if pre_sources and not pre_cited:
        print(
            "错误：接口返回了来源材料，但最终答案中没有 [n] 角标，无法建立核验对应，已拒绝生成。\n"
            "请先修正最终答案（或答案文件）：在关键结论后标注 [1]、[2] 等角标并逐条对应来源材料，再重新运行本脚本。",
            file=sys.stderr,
        )
        raise SystemExit(1)
    # 绑定解析与重编号共用同一逻辑：段落全局编号（现接口）/ 材料级编号（旧接口）皆可
    bound = resolve_citation_targets(base_answer, pre_sources)
    unbound = [cid for cid in pre_cited if cid not in bound]
    if unbound:
        print(
            f"错误：最终答案中的角标 {'、'.join('[' + c + ']' for c in unbound)} 未对应任何来源材料，已拒绝生成。\n"
            "请先修正最终答案：删除该角标，或改挂到真实支撑该结论的材料编号上，再重新运行本脚本。",
            file=sys.stderr,
        )
        raise SystemExit(1)
    self_check_items = normalize_self_check(payload.get("data", payload).get("selfCheck") if isinstance(payload.get("data"), dict) else payload.get("selfCheck"))
    if not self_check_items:
        print(
            "错误：缺少交付前检查结果（--self-check-file），已拒绝生成。\n"
            "请先完成交付前检查并把五项结果写入自检 JSON，再重新运行本脚本。",
            file=sys.stderr,
        )
        raise SystemExit(1)
    failed_checks = [label for label, (status, _) in self_check_items.items() if status != "pass"]
    if failed_checks:
        print(
            f"错误：交付前检查存在未通过项（{'、'.join(failed_checks)}），已拒绝生成。\n"
            "请先修正答案或自检结果至全部通过，再重新运行本脚本；不得带未通过项交付核验报告。",
            file=sys.stderr,
        )
        raise SystemExit(1)
    ...
    # 角标重编号（与报告一致的单一样本源）：被引用角标按首次出现顺序映射为 1..n。
    # 重编号结果同步输出给 Agent：答案文件 + 现成的对话来源清单 + 映射表，
    # 保证"对话回复、核验报告、干净 Markdown"三处编号一致。
    prepared_sources = [dict(s) for s in pre_sources]
    final_answer, prepared_sources, mapping = renumber_citations(base_answer, prepared_sources)
    final_answer_path = SEARCH_RESULTS_DIR / "dknowc_consulting_answer_final.txt"
    final_answer_path.parent.mkdir(parents=True, exist_ok=True)
    final_answer_path.write_text(final_answer, encoding="utf-8")

    # 生成时链接活性检测（对齐公文写作 3.7.0）：404/410 标记失效；连接失败/403
    # 放行不误杀。咨询接口无存档快照，失效链接改用知识专库链接或如实标注。
    if not args.skip_link_check:
        dead = mark_dead_links(prepared_sources)
        if dead:
            print(f"链接检测：{dead} 条原文链接已失效（404/410），将改用知识专库链接或如实标注")

    source_map = {}
    for source in prepared_sources:
        source_map.setdefault(source["id"], source)
    source_list_lines = []
    for cid in citation_ids(final_answer):
        source = source_map.get(cid)
        if not source:
            continue
        bits = [f"[{cid}]《{source.get('title') or '未命名材料'}》"]
        meta = " · ".join(v for v in [source.get("agency"), source.get("date")] if v)
        if meta:
            source_list_lines.append(f"{bits[0]} · {meta}")
        else:
            source_list_lines.append(bits[0])
    mapping_pairs = [f"[{old}]→[{new}]" for old, new in mapping.items() if old != new]
    ...
    output.write_text(render_html(payload, args.title, answer_override=final_answer, question_override=args.question, generated_at=generated_at, sources_override=prepared_sources), encoding="utf-8")
    print(f"已生成：{output}")
    ...
    print(f"已生成重编号答案（对话回复请使用此文件内容）：{final_answer_path}")
    if mapping_pairs:
        print("角标重编号映射：" + "；".join(mapping_pairs))
    print("对话来源清单（已重编号，只含被引用材料，直接用于回复末尾）：")
