...
深知晓办公助手由北京彩智科技有限公司旗下“深知可信智能”提供，是统一覆盖**公文写作、可信咨询、可信检索、PPT 生成**四大类办公场景的综合型 Agent Skill，并采用可扩展架构，未来可持续加入更多能力。本 v1.3.0 为四模块大版本升级版本（溯源核验报告改版 + 注册持久化维持自动写对齐母版 + 公文自由搜索文风参考），四个能力模块分别取自对应独立 Skill 的最新发布包。
...
├── doc-writer/           # 能力一：公文写作（v3.7.0）
├── consulting/           # 能力二：可信咨询（v1.3.0）
├── searching/            # 能力三：可信检索（v1.3.1）
└── ppt-assistant/        # 能力四：PPT 生成（v1.3.0，含 ppt-master MIT 组件）
```
...
| 能力 | 模块 | 来源版本 | 主要交付物 |
| --- | --- | --- | --- |
| 公文写作 | `doc-writer/` | 深知公文写作 v3.7.0 | Word（可选红头）+ 可信核验报告 |
| 可信咨询 | `consulting/` | 深知可信咨询 v1.3.0 | 带角标答案 + 可信核验报告 |
| 可信检索 | `searching/` | 深知可信搜索 v1.3.1 | 直接答案 + 可信核验报告 + 干净 Markdown |
| PPT 生成 | `ppt-assistant/` | 深知可信PPT v1.3.0 | 原生可编辑 .pptx + 双版可信核验报告 |
...
- **PPT 生成**：约束 SVG → 原生 DrawingML 编译（抽取自 ppt-master，MIT，声明见 `ppt-assistant/THIRD_PARTY_NOTICES.md`）；主题/材料/材料免检索三种模式，双确认门工作流，5 种党政合规风格 + 8 种画布，素材层深知可信检索全程溯源；溯源报告自动检测原文链接活性并以存档快照兜底。
