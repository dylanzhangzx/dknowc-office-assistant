...
3. 用户确认后执行（同方案多路**默认并行**：每路独立 `--output`，单批不超过 4 路，单次调用内后台并行、`wait` 全部结束后逐路检查结果；失败的路单独串行重试一次，不得整批反复重发；任意一路额度用尽即整批停止按额度规则处理；平台或接口明显限流〔多路同时报错〕时回退逐路串行。2026-09-20 起对齐公文写作并行口径；原串行约束的知识专库链接已不再进入溯源报告，障碍消除）：
