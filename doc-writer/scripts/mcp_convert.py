...
official-docs/search-results/，再运行本脚本转换——转换产物与 dkag_search.py
直调 REST 的输出同构（content.data.检索文章 / policyFiles / knowledgeBase /
search_meta），研究资料、溯源 JSON 整理与核验报告生成全部按现有流程执行。
...
字段映射：
  ...
  doc_number → 反向构造 policyFiles[{title, writtenText}]（source_note 的
...
def main() -> None:
    ...
    print("  转换产物与 dkag_search.py 直调 REST 同构，研究资料/溯源/报告按现有流程执行。")
